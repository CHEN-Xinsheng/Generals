...
初始地图应保证所有玩家之间的王城是联通的，即从任意一个王城到任意王城，都存在一条不经过山的路径。
...
- 项目运行时，即启动一个抽签器（循环计数器），计数范围是 [0, MAX_RANDOM_TIMER = 128] ，频率是 `Game_Controller` 的 clock 频率，即 50 MHz。
- 按下 `reset` 时，该抽签器置 0。
...
- 如果一方的王城被对方占领，那么对方获胜。
- 如果在第 999 回合结束时，任何一方的王城均未被对方占领，那么
  - 比较双方王城兵力，兵力大者获胜
  - 若兵力相同，判定为平局
